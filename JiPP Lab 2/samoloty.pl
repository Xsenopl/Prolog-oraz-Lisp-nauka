...
loty_wtorki(X) :- lot(X,_,_,_,_,day(_,1,_,_,_,_,_)).
godz_przelotu(Od, Do, Odlot, Przylot) :- lot(_, Od, Do, Odlot, Przylot, _).
operuje(X) :- lot(X,A,B,_,_,_), write(A),write(" "),write(B).
dni_kursu(A,B) :- lot(_,A,B,_,_,day(F,G,H,I,J,K,L)), R = F+G+H+I+J+K+L, write(R).

l¹dowanie_w_godzinach(Lot, B) :-
    lot(Lot,_, B,_, Przylot, _),
    (
        (Przylot >= 800, Przylot =< 1000);
        (Przylot >= 1800, Przylot =< 1859)
    ).
